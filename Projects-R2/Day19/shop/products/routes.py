...
def brands():
    all_brands = Brand.query.join(AddProductDB, (Brand.id == AddProductDB.brand_id)).all()
    return all_brands


def categories():
    all_categories = Category.query.join(AddProductDB, (Category.id == AddProductDB.category_id)).all()
...
def result():
    ...
    products = AddProductDB.query.msearch(search_word, fields=['name', 'description'], limit=6)
