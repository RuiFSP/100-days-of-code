...
@app.route('/')
def home():
    products = AddProductDB.query.filter(AddProductDB.stock > 0)
...
def get_brand(id):
    brand = AddProductDB.query.filter_by(brand_id=id)
    brands = Brand.query.join(AddProductDB, (Brand.id == AddProductDB.brand_id)).all()
...
# TODO: Delete Product not working when i press button
